...
if ($_POST){
	...
	if ($sra_id != '') {
    $query1 = "update eti_approve_details set approve_b = '4', approve_b_desc = '$clarify_level_desc_a',
			   approve_b_date_time='$current_date',approve_b_email='$clarify_email', approve_b_name='$clarify_name' where id = '$sra_id'";
	...
	$query66 = "insert into eti_clarify_details (sra_id,clarify_a,clarify_a_desc,clarify_a_date_time,clarify_a_email,clarify_a_name,chat_count,clarify_level) 
				values('$eti_id','3','$clarify_level_desc_a','$current_date','$clarify_email','$clarify_name','$chat_count','2')";
		...
		$body ='<html>
				...
				font-weight: bold;
					...
					<body>
						...
						<div>
							<a class="link_button" href="'.$base_url.'approve_b.php?eti_id='.base64_encode($eti_id).'&&email='.base64_encode($email).'&&email_name='.base64_encode($email_name).'" target="_blank">Approve</a>
							<a class="link_button" href="'.$base_url.'reject_b.php?eti_id='.base64_encode($eti_id).'&&email='.base64_encode($email).'&&email_name='.base64_encode($email_name).'" target="_blank">Reject</a>
							<a class="link_button" href="'.$base_url.'clarify_b.php?eti_id='.base64_encode($eti_id).'&&email='.base64_encode($email).'&&email_name='.base64_encode($email_name).'" target="_blank">Clarification</a>
						 </div><br/>
						 <div> 
...
<div class="example-modal">
        <div class="modal modal-primary">
          <div class="modal-dialog">
		    <form action="clarified_b.php" name="clarify" id="clarify" onsubmit="return validateForm()" method="post">
