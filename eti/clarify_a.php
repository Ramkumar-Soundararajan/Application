<?php
	...
	if($invoice_type != 'Advance (Normal)' || $invoice_attachment != 'None' || ($credit_term == '45 Days' || $credit_term == '60 Days' || $credit_term == '90 Days' || $credit_term == '120 Days') || ($billing_frequency == 'Monthly' && $price_accept <= 2000)){
		...
		$emp_name_condition = $res122['name'];
		$emp_user_mail_condition = $res122['email'];
	}
	...
	if ($sra_id != '') {
		$query1 = "update eti_approve_details set approve_a = '3', approve_a_desc = '$clarify_level_desc_a',
				   approve_a_date_time='$current_date',approve_a_email='$email', approve_a_name='$email_name' where id = '$sra_id'";
		...
		$body ='<html>
				...
				font-weight: bold;
					...
					<body>
						...
						<div>
							<a class="link_button" href="'.$base_url.'clarified_a.php?eti_id='.base64_encode($eti_id).'&&email='.base64_encode($email).'&&email_name='.base64_encode($email_name).'&&clarify_email='.base64_encode($cla_employee_email).'&&clarify_name='.base64_encode($cla_employee_name).'" target="_blank">Clarify</a>
						 </div><br/>
						 <div> 
			...
			$query111 = "update eti_sra_status set approve_desc = '$approve_desc', approve_date_time = '$current_date',approve_email_id='$clarify_email',approve_name='$clarify_name', user_id='' where sra_id = '$eti_id'";
			$exec111 = mysql_query($query111) or die ("Error in Query111".mysql_error());
...
<div class="example-modal">
        <div class="modal modal-primary">
          <div class="modal-dialog">
		    <form action="clarify_a.php" name="clarify" id="clarify" onsubmit="return validateForm()" method="post">
