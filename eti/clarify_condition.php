...
if (isset($_REQUEST["email_name"])) { $email_name = base64_decode($_REQUEST["email_name"]); } else { $email_name = ""; }
	...
	if($approve_condition == 3){
		header ("location:message.php?st=".base64_encode(6));
		exit;
	}
	
$query121 = "select a.employee_name as employee_name,a.user_mail as user_mail from eti_portal_user a, eti_sra b where a.id = b.created_by and b.id= '$eti_id'";
...
if ($_POST){
	$clarify_level_desc_a = mysql_real_escape_string($_POST['clarify_level_desc_a']);
	$eti_id = $_POST['eti'];
	$email = $_POST['email'];
	...
	$query77 = "select id,employee_name,user_mail from eti_portal_user where user_mail='$clarify_email'";
	...
	if ($sra_id != '') {
    $query1 = "update eti_approve_details set approve_condition = '3', approve_condition_desc = '$clarify_level_desc_a',approve_condition_date_time='$current_date',approve_condition_email='$email', approve_condition_name='$email_name' where id = '$sra_id'";
	$exec1 = mysql_query($query1) or die ("Error in Query1".mysql_error());
	} else {
	$query6 = "insert into eti_approve_details (sra_id,approve_condition,approve_condition_desc,approve_condition_date_time,approve_condition_email,approve_condition_name)values('$eti_id','3','$clarify_level_desc_a','$current_date','$email','$email_name')";
	...
	$query66 = "insert into eti_clarify_details (sra_id,clarify_a,clarify_a_desc,clarify_a_date_time,clarify_a_email,clarify_a_name,chat_count,clarify_level) 
				values('$eti_id','3','$clarify_level_desc_a','$current_date','$email','$email_name','$chat_count','1')";
	...
	$created_employee_email = $res388['user_mail'];
		...
		$body ='<html>
				...
				font-weight: bold;
					...
					<body>
						<div>
						<b>Hi '.$cla_employee_name.',</b>
						</div> <br /> 
						<div>Please find ETI PDF attached below.</div> <br />
						<div><b>GM:</b> '.$percentage.'</div> <br />
						<div><b>Description:</b> '.$clarify_level_desc_a.'</div> <br />
						<div>
							<a class="link_button" href="'.$base_url.'clarified_condition.php?eti_id='.base64_encode($eti_id).'&&email='.base64_encode($email).'&&email_name='.base64_encode($email_name).'&&clarify_email='.base64_encode($cla_employee_email).'&&clarify_name='.base64_encode($cla_employee_name).'" target="_blank">Clarify</a>
						 </div><br/>
						 <div> 
						<b>Regards,<br /> Rentokil Intial Pvt Ltd.</b></div>
					</body>
				</html>';
			//$subject = "ETI Request Form (".$serial_number." - ".$name.")";
			...
			$query111 = "update eti_sra_status set approve_desc = '$approve_desc', approve_date_time = '$current_date',approve_email_id='$email',approve_name='$email_name', user_id='$user_id' where sra_id = '$eti_id'";
			$exec111 = mysql_query($query111) or die ("Error in Query111".mysql_error());
...
<div class="example-modal">
        <div class="modal modal-primary">
          <div class="modal-dialog">
		    <form action="clarify_condition.php" name="clarify" id="clarify" onsubmit="return validateForm()" method="post">
